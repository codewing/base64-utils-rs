...
fn encode(file_path: PathBuf) {
    ...
    let encoded = base64::engine::general_purpose::GeneralPurpose::encode(&general_purpose::STANDARD_NO_PAD, &file_content);
    println!("Encoded File:\n{}", encoded);
}

fn decode(text: String) {
    let decoded = base64::engine::general_purpose::GeneralPurpose::decode(&general_purpose::STANDARD_NO_PAD, &text).unwrap();
